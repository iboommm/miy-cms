                ...
                <script>
                   $(document).ready(function() {
                        ...
                        id = btoa(" `id` = " + id + "");
                        $.getJSON('genJSON/shop_brand/'+id, function(json, textStatus) {
                               $('#name').val(json.shop_brand[0].name);
                    ...
                    function deleteUser(id) {
                        ...
                        $.post('admin.php?mode=remove-brand',{data: genData,sending: 'remove-brand'}, function(data, textStatus, xhr) {
                            if(data == 1) {
                                $('#status-delete').html("<div class=\"alert alert-success\"><strong><?php echo $lang["Admin-success"]; ?>!</strong> Remove Complete</div>");
                                setTimeout(function() {
                                    location.href = "admin.php?mode=list-brand";
                                }, 1000);
                ...
                <script>
                    ...
                    $('#confirm').click(function(event) {
                        ...
                        var genData = name + "," + id;
                        $.post('admin.php?mode=edit-brand', {data: genData,sending: "edit-brand"}, function(data){
                          if(data == 1) {
                                var name = $('#name').val();
